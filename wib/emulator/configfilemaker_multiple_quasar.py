#Config file maker used when multiple quasar servers are used due multiple wibs emulators are turned on.
import argparse
...
parser.add_argument('--wib_num_start', type = int, required = True, help = 'first wib number')
...
wib_n_start = args.wib_num_start

template = """\
    <WIB zmq_endpoint="tcp://localhost:{port}" name="wib_{i}" poll_period="1">
...
with open("config.xml", "w") as file:  
    ...
    for i, port in enumerate(ports, start=wib_n_start):
